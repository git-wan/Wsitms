Ext.define('Wsitms.controller.Root', {
	...
	onMenuSelectForRout : function(tree, record, index, eOptes) {
		var flag = this.redirectTo(record.get('moduleId')) //将获得的ID与路由器匹配，返回true，路由存在不执行方法，返回false，路由存在执行方法
		//防止路由存在，但是页面关闭，需重新加载页面
		if(!flag){
			var contentPanel = this.getContentPanel();
			tab = contentPanel.items.getByKey(id);
			if (!tab) {
				this.handleRout(record.get('moduleId'))
			}
		}
	},
	
	//第二步，路由匹配，进入before 方法
